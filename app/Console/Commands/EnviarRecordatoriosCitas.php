...
class EnviarRecordatoriosCitas extends Command
{
     ...
     */
    protected $description = 'Envía recordatorios por correo a los clientes (un día antes) y resúmenes diarios en PDF a barberos y administradores';
    ...
    public function handle(): int
    {
        ...
        $this->info("🔍 Procesando agenda central:");
        $this->info("📅 Fecha de Hoy (Reportes diarios): {$hoy}");
        $this->info("📅 Fecha de Mañana (Recordatorios de clientes): {$manana}");

        // ==================== 1. RECORDATORIOS A CLIENTES (UN DÍA / 24 HORAS ANTES) ====================
        ...
        foreach ($citasClientes as $cita) {
            try {
                ...
                $this->info("📧 Recordatorio de mañana enviado al cliente: {$cita->email} ({$cita->nombre_cliente})");
                $enviadosClientes++;
            } catch (\Exception $e) {
        ...
        // ==================== 2. RESUMEN DIARIO A BARBEROS (CITAS DE HOY) ====================
        ...
        // ==================== 3. RESUMEN DIARIO CONSOLIDADO A ADMINISTRADORES (TODAS LAS CITAS DE HOY) ====================
        ...
        $this->newLine();
        $this->info("=== RESUMEN DE ENVÍO DE CORREOS ===");
        $this->info("✅ Recordatorios de Clientes Enviados (Mañana): {$enviadosClientes} (Errores: {$erroresClientes})");
