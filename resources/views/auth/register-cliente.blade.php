...
<head>
    ...
    <title>Registro de Cliente - BarberShop</title>
...
<body class="bg-black text-white min-h-screen flex flex-col justify-center items-center px-4 relative overflow-hidden">
    ...
    <div class="max-w-md w-full z-10">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-yellow-500 mb-2">Registro de Cliente</h1>
            <p class="text-gray-400 text-sm">Completa tus datos para crear tu cuenta y agendar tus citas.</p>
        </div>

        <form method="POST" action="{{ route('register.cliente.store') }}" class="bg-[#0f0f0f] border border-gray-800 p-8 rounded-2xl shadow-xl space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required placeholder="cliente@correo.com"
                       class="w-full bg-black border border-gray-700 rounded-xl px-4 py-3 focus:outline-none focus:border-yellow-500 text-white transition">
            </div>
        ...
        <div class="mt-6 text-center space-y-2">
            <p class="text-gray-500 text-sm">¿Prefieres registrarte como Barbero? 
                <a href="{{ route('register.barbero') }}" class="text-yellow-500 hover:underline">Haz clic aquí</a>
            </p>
